...
#pragma region "resize regressions array"
    qPeaks::validRegression_static *
    qPeaks::resize_validRegressions_static(
        validRegression_static *validRegressions,
        const int n_regressions,
        const int n_regressions_new)
    {
      validRegression_static *newValidRegressions = new validRegression_static[n_regressions_new];
      std::copy(validRegressions, validRegressions + n_regressions, newValidRegressions);
      delete[] validRegressions;
      return newValidRegressions;
    }
#pragma endregion "resize regressions array"
        ...
        const float retentionTime)
    {
      ...
      for (auto it_separators = treatedData.separators.begin(); it_separators != treatedData.separators.end() - 1; it_separators++)
      {
        ...
        else
        {
          ...
          X = new float[n];
          df = new bool[n];
          validRegressions = new validRegression_static[2048];
        }
        ...
        // if (isStatic)
        // {
        runningRegression_static(Y, Ylog, df, n, validRegressions, validRegressionsIndex, validRegressionsSize);
...
#pragma region "running regression static"
        ...
        int &validRegressionsSize)
    {
      ...
      for (int scale = 2; scale <= maxScale; scale++)
      {
        ...
        __m128 *beta = nullptr;           // coefficients matrix
        bool is_dynamic = false;
        if (n > 512)
        {
          beta = new (std::align_val_t(16)) __m128[n_segments];
          is_dynamic = true;
          convolve_dynamic(scale, ylog_start, n, beta); // do the regression
        }
        else
        {
          alignas(16) __m128 beta_fixed[512];                // coefficients matrix
          convolve_static(scale, ylog_start, n, beta_fixed); // do the regression
          beta = beta_fixed;
        }
        validateRegressions_static(beta, n_segments, y_start, ylog_start, df_start, scale, validRegressionsIndex, validRegressionsSize, validRegressions);
        if (is_dynamic)
        {
          ::operator delete[](beta, std::align_val_t(16));
        }
      } // end for scale loop
...
#pragma region "validate regressions static"
        ...
        validRegression_static *validRegressions)
    {
      ...
      // declare variables
      validRegression_static *validRegressionsTmp = nullptr; // temporary vector to store valid regressions initialized with random states
      int validRegressionsTmpSize = 512;                     // size of the valid regressions vector
      if (n_segments > 512)
      {
        validRegressionsTmp = new validRegression_static[n_segments]; // temporary vector to store valid regressions initialized with random states
        validRegressionsTmpSize = n_segments;                         // size of the valid regressions vector
      }
      else
      {
        validRegression_static validRegressionsTmp_fixed[512]; // temporary vector to store valid regressions initialized with random states
        validRegressionsTmp = validRegressionsTmp_fixed;       // temporary vector to store valid regressions initialized with random states
      }
      ...
      for (int i = 0; i < n_segments; i++)
      {
        ...
        */
        if (validRegressionsIndexTmp > validRegressionsTmpSize - 1)
        {
          validRegressionsTmp = resize_validRegressions_static(validRegressionsTmp, validRegressionsTmpSize, validRegressionsTmpSize + 512);
          validRegressionsTmpSize += 512;
        }
        validRegression_static &reg = validRegressionsTmp[validRegressionsIndexTmp];
        reg.index_x0 = i + scale;                      // index of the center of the window (x==0) in the Y matrix
        reg.scale = scale;                             // scale
        reg.df = df_sum - 4;                           // df
        reg.apex_position = apex_position + i + scale; // apex position in x-axis 0:n
        reg.mse = 0;                                   // initial MSE
        reg.coeff = coeff;                             // coefficients register
        reg.isValid = true;                            // isValid
        reg.left_limit = left_limit;                   // left_limit
        reg.right_limit = right_limit;                 // right_limit
        reg.area = area;                               // peak area
        reg.uncertainty_area = uncertainty_area;       // uncertainty of the peak area
        reg.uncertainty_pos = uncertainty_pos;         // uncertainty of the peak position
        reg.uncertainty_height = uncertainty_height;   // uncertainty of the peak height
        validRegressionsIndexTmp++;
      } // end for loop
      if (validRegressionsIndex + validRegressionsTmpSize > validRegressionsSize)
      {
        validRegressions = resize_validRegressions_static(validRegressions, validRegressionsSize, validRegressionsSize + validRegressionsTmpSize);
        validRegressionsSize += validRegressionsTmpSize;
      }
...
#pragma region "add peak properties"
        ...
        const int scanNumber)
    {
      ...
      if (*mz_start != -255.f)
      {
        ...
        peak.mzUncertainty = (tValuesArray[regression.df-1] * regression.uncertainty_pos * sqrt(1 + 1/(regression.df+4))) * dmz;
      }
      else
      {
